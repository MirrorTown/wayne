...
func (c *APIController) HarborClient(host string, userName string, passwd string) (*harbor.Client, error) {
	harborCli, exist := harborManagerSets.Load(host)
	//如果不存则重新获取
	if !exist {
		client, err := harbor.NewClient(nil, host, userName, passwd, true)
		if err != nil {
			c.AbortBadRequestFormat(fmt.Sprintf("Get harbor client error. %v", err))
		}
		harborManagerSets.Store(host, client)
		harborCli, _ = harborManagerSets.Load(host)
	}

	return harborCli.(*harbor.Client), nil;
}
